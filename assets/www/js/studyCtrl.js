...
c.controller('StudyCtrl',['$scope', '$http', 'helperService','$localstorage', '$stateParams','$rootScope', '$timeout', 'ionicMaterialInk', 'ionicMaterialMotion', 'ngProgressFactory', '$ionicModal', '$ionicPopup','$window','$ionicLoading','$state', function($scope,$http, helper,$localstorage, $stateParams,$rootScope, $timeout, ionicMaterialInk, ionicMaterialMotion, ngProgressFactory, $ionicModal, $ionicPopup, window,$ionicLoading,$state) {
    // Set Header
    ...
    $scope.$parent.setHeaderFab('left');
     $ionicLoading.show({
        template: 'loading...<br><img style="width: 40%" src="img/loading.gif">'
      });
    ...
    // Triggered on a button click, or some other target
      ...
      $scope.showConfirm = function(animation) {
        ...
        $scope.quizName = 'data/agriculture.js';
    ...
    $scope.onSubmit = function () {
       ...
       $ionicLoading.show({
        template: 'marking...<br><img style="width: 40%" src="img/loading.gif">'
      });
        ...
        $timeout(function() {
               $ionicLoading.hide();
               $scope.mode = 'result';
             window.localStorage['noOfPracticeTaken']++;
         }, 1000);        
    }
    ...
    $scope.getCorrectAns = function (question) {
        var result = 'correct';
        question.Options.forEach(function (option, index, array) {            
                if (option.IsAnswer) {
                    correctAns = option.Name;
